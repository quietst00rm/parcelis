      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {integrations.map((item, idx) => {
                ...
                const CardWrapper = isShopify ? 'a' : 'div';
                const cardProps = isShopify ? {
                ...
                return (
                  <CardWrapper 
                    key={idx} 
                    {...cardProps}
                    className="bg-white rounded-2xl p-8 flex flex-col items-center text-center hover:shadow-xl transition-all duration-300 group cursor-pointer border border-transparent hover:border-blue-100 transform hover:-translate-y-1"
                  >
